...
Function code **0x03** (Read Holding Registers) and **0x06** (Write Single Register) are implemented.
...
| 42000 | `0xA410` | RS485 control mode | `u16` | Yes | R/W: `0x55AA` enable, `0x55BB` disable |
| 42010 | `0xA41A` | Forcible charge/discharge | `u16` | Yes | R/W: `0` stop, `1` charge, `2` discharge |
| 42020 | `0xA424` | Forcible charge power | `u16`, W | Yes | R/W: `0-2500` |
| 42021 | `0xA425` | Forcible discharge power | `u16`, W | Yes | R/W: `0-2500` |
| 32200 | `0x7DC8` | AC voltage | `u16`, 0.1 V | Yes | Returns `2300` (230.0 V); supports quantity `1` or `5` |
| 32201 | `0x7DC9` | AC current | `u16`, 0.01 A | Yes | Magnitude calculated from active power / 230 V |
| 32202 | `0x7DCA` | AC power | `s32`, W | Yes | Negative when charging, positive when discharging; based on forced power setting |
| 32204 | `0x7DCC` | AC frequency | `u16`, 0.01 Hz | Yes | Returns `5000` (50.00 Hz) |
...
## Serial commands (ESP32 firmware)

The ESP32 firmware (`src/Marstek-ModBus-Gateway.ino.cpp`) accepts these commands over USB serial:

| Command | Action |
|---|---|
| `charge=<watts>` | Enable RS485 control mode, set charge power, start charging |
| `discharge=<watts>` | Enable RS485 control mode, set discharge power, start discharging |
| `charge=0` / `discharge=0` / `stop` | Stop charge/discharge and disable RS485 control mode |

Example: `charge=2000` writes `0x55AA` to `42000`, `2000` to `42020` and `1` to `42010`.
...
2. **Static/example values**: all register values are hard-coded examples taken from the CSV. Telemetry (voltage/current/power) does not yet change when charge/discharge control is active.
3. **FC 0x03 and FC 0x06 supported**: read holding registers and write single register are implemented. Other function codes (0x04, 0x10, etc.) return an illegal-function exception.
...
6. **Firmware loop**: `src/Marstek-ModBus-Gateway.ino.cpp` reads telemetry every 4 s and processes serial commands (`charge=<watts>`, `discharge=<watts>`, `stop`) immediately.
...
- [x] 42000, 42010..42021 — RS485 control / force charge-discharge
- [ ] 43000..43129 — Time-of-use settings
